# Day 9 — LLM 三层容错、任务状态机与双 Agent 架构

## 今日改动

完成 LLM 调用的三层容错体系、企业级任务状态机扩展、基于页面复杂度的模型路由，以及 Planner + Executor 双 Agent 协作架构，核心工作分五部分：
...
5. **Planner + Executor 双 Agent 架构**：`enterprise/agent/` 目录实现了多 Agent 协作系统——`PlannerAgent` 接收导航目标，通过 LLM 拆解为有序子任务列表（每个子任务包含目标描述、完成条件、最大重试次数、失败策略）；`ExecutorAgent` 逐步执行子任务并返回结构化结果；`AgentCoordinator` 编排两者的通信，维护整体任务状态，处理四种失败策略（retry/skip/abort/replan）。失败时 Coordinator 请求 Planner 重新规划剩余步骤，而非终止整个任务。子任务状态通过 Pydantic 模型持久化，支持断点续跑。双 Agent 架构接入审计回调，子任务粒度的操作记录单独标注。重规划次数超过上限（默认 3 次）后自动转入 NEEDS_HUMAN 状态

新增测试 84 个（Day 9），累计 405 + 24 = 429 个，全部通过。
...
**Planner + Executor 分离而非单一 Agent**：原始 Skyvern 是单一 Agent 循环执行，遇到复杂金融流程（跨多个页面、多个系统跳转）时，一旦中间某步失败，整个任务从头重来。双 Agent 方案将"规划"和"执行"解耦：Planner 负责理解目标并分解为子任务序列，Executor 负责具体执行。这使得失败恢复的粒度从"整个任务"细化到"单个子任务"。

**四种失败策略的设计**：不同子任务的失败影响不同，不能一刀切处理：
- **retry**：瞬态错误（网络超时、页面加载慢），重试通常可以解决
- **skip**：非关键步骤（关闭弹窗、忽略广告），失败不影响核心逻辑
- **abort**：关键前置步骤（登录失败），后续步骤没有继续的意义
- **replan**：路径阻塞（按钮不存在、页面结构变化），需要 Planner 重新想一条路

**重规划上限（max_replans=3）**：避免 Planner 和 Executor 在无解的任务上无限循环消耗 LLM 调用。超出上限后转入 NEEDS_HUMAN 状态，由人工判断是任务定义有问题还是系统环境异常。

**LLM 不可用时的降级**：Planner 在 LLM 调用失败时自动回退到单步计划（把整个导航目标作为一个子任务），确保系统在 LLM 服务不可用时仍然可以执行简单任务。
...
**复杂金融流程的断点续跑**：银行的日终批处理、保险的批量核保等场景中，一个任务可能包含数十个子步骤，中途因网络中断或系统维护暂停。双 Agent 架构通过子任务状态持久化（`completed_subtasks` 列表），支持从上一个成功的子任务继续执行，而非从头开始。这在降低重复操作成本的同时，也减少了对目标系统的无意义访问压力。

**子任务粒度的审计追踪**：审计回调在每个子任务执行后触发，记录子任务目标、执行结果、耗时和错误信息。当监管机构要求回溯某笔操作的完整链路时，系统可以精确到"在第几步做了什么、结果如何、失败后重新规划了哪些步骤"。
